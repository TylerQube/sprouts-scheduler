...
class Volunteer:
    ...
    def load_availability(self, form, shifts):
        for column, value in form.items():
            ...
            day = column[column.index(":")+1::].strip().replace("[", "").replace("]", "")
            ...
            for t in times:
                for s in shifts:
                    if (
                        ...
                        or len(day) > 0 and s.day not in day.lower() 
                        or t != s.time 
                    ):
                        continue
                    # add to availability
...
def load_volunteers(table, shifts):
    ...
    for index, row in table.iterrows():
        ...
        existing = next((v for v in vollies if v.name == vol.name and v.email == v.email), None)
        if existing:
